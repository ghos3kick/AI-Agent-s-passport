...
export function parsePassportFromNftItem(item: {
    address: string;
    index: number;
...
export function parsePassportDataFromGetMethod(
    stack: Array<{ type: string; num?: string; cell?: string; slice?: string }>,
) {
    // get_passport_data returns PassportData struct:
    // owner (address), capabilities (string/cell), endpoint (string/cell),
    // metadataUrl (string/cell), createdAt (int), txCount (int), revokedAt (int)
    return {
        ownerAddress: parseAddressFromStack(stack[0]),
        capabilities: stack[1]?.cell ?? stack[1]?.slice ?? '',
        endpoint: stack[2]?.cell ?? stack[2]?.slice ?? '',
        metadataUrl: stack[3]?.cell ?? stack[3]?.slice ?? '',
        createdAt: Number(parseNumFromStack(stack[4])),
        txCount: Number(parseNumFromStack(stack[5])),
        revokedAt: Number(parseNumFromStack(stack[6])),
    };
}
